...
def pipeline(common) {
    def clusterName = "blink-per-commit-${BUILD_ID}"
    def clusterSize = 5
    def clusterIps;
    ...
    try {
            ...
            timeout(20) {
                parallel 'provision-backends' : {
                    ...
                    dir ('blink') {
                        ...
                        sh './node_modules/protractor/bin/webdriver-manager start > /dev/null 2>&1 &'
                    }
                }, 'launch-xvfb' : {
        ...
        stage('Run blink tests') {
            timeout(25) {
                dir('blink') {
                    ...
                    sh "grunt e2eProtractorFlake --hosts=${hosts}"
                }
            }
        }
    } finally {
        ...
        try {
            ...
            stage('Report results') {
                junit healthScaleFactor: 0.0, testResults: 'blink/test/results/protractor-e2e*.xml'
                processBlinkRunResults()
            }
