...
// @THOUGHTSPOT_CUSTOMIZATION
// Note (sunny): We have modified this file for performance improvements.
// Please see the git history for changes.
    ...
    mod.directive('sfVirtualRepeat', ['$log', '$rootElement', function($log, $rootElement){
        ...
        function sfVirtualRepeatCompile(element, attr, linker) {
            ...
            function sfVirtualRepeatPostLink(scope, iterStartElement, attrs){
                ...
                function makeNewScope (idx, containerScope) {
                    var childScope = containerScope.$new();
                    childScope[ident.value] = containerScope.collection[idx];
                    childScope.$index = idx;
                    childScope.$first = (idx === 0);
                    // THOUGHTSPOT_CUSTOMIZATION_BEGIN
                    // Set @$virtualLast on the last rendered element.
                    childScope.$virtualLast = (idx == state.firstActive + state.active - 1);
                    // THOUGHTSPOT_CUSTOMIZATION_END
                    childScope.$last = (idx === (containerScope.collection.length - 1));
                    childScope.$middle = !(childScope.$first || childScope.$last);
                    childScope.$watch(
                        function() {
                            return containerScope.collection[idx];
                        },
                        function(newValue, oldValue){
                            if (newValue !== oldValue) {
                                childScope[ident.value] = newValue;
                            }
                        }
                    );
                    return childScope;
                ...
                function addElements (start, end, containerScope, insPoint) {
                    var frag = document.createDocumentFragment();
                    var newElements = [], element, idx, childScope;
                    for( idx = start; idx !== end; idx ++ ){
                        childScope = makeNewScope(idx, containerScope);
                ...
                function sfVirtualRepeatWatchExpression(scope){
                    var coll = scope.collection = scope.$eval(ident.collection);
                ...
                function sfVirtualRepeatListener(newValue, oldValue, scope){
                    ...
                    if( newValue === oldValue ){
                        ...
                        newElements = addElements(newValue.start, oldEnd, scope, iterStartElement);
                        ...
                        if( !contiguous ){
                            ...
                            rendered = addElements(newValue.start, newEnd, scope, iterStartElement);
                        }else{
                            if( forward ){
