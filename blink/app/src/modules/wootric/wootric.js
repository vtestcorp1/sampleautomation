...
addBooleanFlag('testNPS', 'Add a flag that enables wootric for testing', false);

blink.app.factory('wootric', ['routeService',
    'sessionService',
    function (routeService,
              sessionService) {
        window.wootricSettings = {
            account_token: 'NPS-573079d9' // This is your unique account token.
        };
        ...
        return {
            run: function() {
                var createTime = sessionService.getUserCreateTime();
                if (!createTime) {
                    return;
                }
                if (flags.getValue('testNPS')) {
                ...
                window.wootricSettings.created_at = createTime;
                window.wootricSettings.email = sessionService.getUserGuid();

                window.wootricSettings.properties = {
                    privilegeBitMap: sessionService.getUserPrivilegesBitMap(),
                    clusterName: sessionService.getClusterName()
                };
                window.wootric("run");
            }
        }
